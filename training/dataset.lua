...
function dataset:sampleTriplet(quantity)
   ...
   for i=1,quantity do
      ...
      if opt.negOnlyFromSameImage then
        local posClassBaseName = self.classes[posClass]:match('(.*_).*'):gsub("([^%w])", "%%%1")
        
        -- find all classes that start with this base class
        local potential_classes = {}
        local potential_classes_offset = -1
        for i=1,#self.classes do     
          if (self.classes[i]:find(posClassBaseName)) then 
            if (-1 == potential_classes_offset) then
              potential_classes_offset = i - 1
            end
            table.insert(potential_classes, self.classes[i])
          end
        end
         
        while negClass == posClass do
          negClass = torch.random(1, #potential_classes) + potential_classes_offset
        end
      else
      --------------------------- NEW STUFF ENDS HERE ---------------------------
        while negClass == posClass do
...
function dataset:samplePeople(peoplePerBatch, imagesPerPerson)
  ...
  local classes
  if opt.negOnlyFromSameImage then
    -- randomly sample one class name
      local rand_class = self.classes[torch.randperm(#self.classes)[{{1}}][1]]

      -- extract the base name from this class
      local rand_class_base_name = rand_class:match('(.*_).*'):gsub("([^%w])", "%%%1")

      -- find all classes that start with this base class
      local potential_classes = {}
      local potential_classes_offset = -1
      for i=1,#self.classes do     
        if (self.classes[i]:find(rand_class_base_name)) then 
          if (-1 == potential_classes_offset) then
            potential_classes_offset = i - 1
          end
          table.insert(potential_classes, self.classes[i])
      ...
      classes = torch.randperm(#potential_classes)[{{1,peoplePerBatch}}]:int() + potential_classes_offset
  else
      classes = torch.randperm(#self.classes)[{{1,peoplePerBatch}}]:int()
  end
  --------------------------- NEW STUFF ENDS HERE ---------------------------

   --local classes = torch.randperm(#self.classes)[{{1,peoplePerBatch}}]:int()
   --peoplePerBatch = math.min(peoplePerBatch, #potential_classes)
   --print(peoplePerBatch)
